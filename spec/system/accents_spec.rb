...
RSpec.describe "アクセント登録機能", type: :system do
  ...
  context 'アクセント登録ができるとき' do
    it 'ログインしたユーザーはアクセント新規登録できる' do
      ...
      expect{
        click_on "登録する"
      }.to change{ Accent.count }.by(1)
      # 詳細ページに遷移していることを確認する
      expect(page).to have_no_content('登録')
      expect(page).to have_content('コメント')
      # 詳細ページに登録した内容が反映されていることを確認する
      expect(page).to have_content("#{@accent.word}")
      expect(page).to have_content("テ＼スト")
      expect(page).to have_content("#{@accent_part_of_speech}")
      expect(page).to have_content("#{@accent_accent_pattern}")
      expect(page).to have_content("#{@accent.info}")
    end
  end
...
RSpec.describe "アクセント編集機能", type: :system do
  ...
  context 'アクセント編集ができるとき' do
    it 'ログインしたユーザーは自分が登録した単語の編集ができる' do
      ...
      ).to eq "#{@accent1.part_of_speech_id}"
      expect(
        find('#speech-select-id').value
      ).to eq "#{@accent1.part_of_speech_id}"
      expect(
        find('#accent-pattern-id').value
      ).to eq "#{@accent1.accent_pattern_id}"
      expect(
        find('#beat-count-id').value
      ).to eq "#{@accent1.beat_count_id}"
      expect(
        find('#info-id').value
      ).to eq @accent1.info
      # 投稿内容を編集する
      ...
      expect(page).to have_content("#{@accent_part_of_speech}")
      expect(page).to have_content("#{@accent_accent_pattern}")
      expect(page).to have_content("#{@accent1.info}")
    end
  end
